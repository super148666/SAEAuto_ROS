/* Copyright 2009 Terraneo Frederico
...
void AsyncSerial::open(const std::string& devname, unsigned int baud_rate,
    ...
    thread t(boost::bind(&boost::asio::io_service::run, &pimpl->io));
...
bool AsyncSerial::errorStatus() const {
    boost::lock_guard<mutex> l(pimpl->errorMutex);
    return pimpl->error;
}
...
void AsyncSerial::write(const char *data, size_t size) {
    {
        boost::lock_guard<mutex> l(pimpl->writeQueueMutex);
...
void AsyncSerial::readEnd(const boost::system::error_code& error, size_t bytes_transferred) {
    if (error) {
        #ifdef __APPLE__
        if (error.value() == 45) {
          // FIXME: is this really necessary? to code is never going to run on apple anyways
...
void AsyncSerial::doWrite() {
    ...
    if (pimpl->writeBuffer == 0) {
        ...
        copy(pimpl->writeQueue.begin(), pimpl->writeQueue.end(), pimpl->writeBuffer.get()); pimpl->writeQueue.clear();
        async_write(pimpl->port, boost::asio::buffer(pimpl->writeBuffer.get(), pimpl->writeBufferSize), boost::bind(&AsyncSerial::writeEnd, this, boost::asio::placeholders::error));
    }
}
